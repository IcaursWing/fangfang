...
public class BluetoothLeService extends Service {
    ...
    public String bytesToHexString(byte[] src)// 例子byte[]：0x11,0x22,0x33,0x44,0x55,0x66
    // 转换后String:“112233445566”
    {
    ...
    public String bytesToHexString1(byte[] src)// 例子byte[]：0x11,0x22,0x33,0x44,0x55,0x66
    // 转换后String:“11 22 33 44 55 66”
    {
        StringBuilder stringBuilder = new StringBuilder(src.length);
        for (byte byteChar : src)
            stringBuilder.append(String.format(" %02X", byteChar));
    ...
    public String bytesToHexString1(byte[] src, int index)// 例子byte[]：0x11,0x22,0x33,0x44,0x55,0x66
    // 转换后String:“11 22
    // 33 44 55 66”
    {
        if (src == null)
    ...
    public String String_to_HexString0(String str)// 例子String“123456”
    // 转换后String：“313233343536”
    {
    ...
    public String String_to_HexString(String str)// 例子String“123456”
    // 转换后String：“31 32 33 34 35
    // 36”
    {
        String st = str.toString();
        byte[] WriteBytes = new byte[st.length()];
    ...
    public byte[] String_to_byte(String str)// 例子String“123456”
    // 转换后byte[]：0x31,0x32,0x33,0x34,0x35,0x36
    {
        String st = str.toString();
        byte[] WriteBytes = new byte[st.length()];
        return WriteBytes;
    ...
    public String byte_to_String(byte[] byt)// 例子byte[]：0x31,0x32,0x33,0x34,0x35,0x36
    // 转换后String:“123456”
    {
    ...
    public String byte_to_String(byte[] byt, int index)// 例子byte[]：0x31,0x32,0x33,0x34,0x35,0x36
    // 转换后String:“123456”
    {
        if (byt == null)
            return null;
    ...
    public int txxx(String g, boolean string_or_hex_data) {
        ...
        if (string_or_hex_data)
            WriteBytes = g.getBytes();// getBytesByString( g );//
            // hex2byte(g.toString().getBytes());
        else
        ...
        if (data_len_20 > 0) {
            for (; i < data_len_20; i++) {
                ...
                gg = mBluetoothGatt.getService(UUID.fromString(Service_uuid))
                        .getCharacteristic(
                                UUID.fromString(Characteristic_uuid_TX));
                gg.setValue(da);
    ...
    public void function_data(byte[] data) {
        ...
        gg = mBluetoothGatt.getService(UUID.fromString(Service_uuid))
                .getCharacteristic(
                        UUID.fromString(Characteristic_uuid_FUNCTION));
        // byte t[]={51,1,2};
    ...
    // BluetoothGattDescriptor dsc =ale.getDescriptor(UUID.fromString(
        ...
        try {
            // if( p )
            {
                ...
                switch (p) {
                    case 0:// 0xFFE1 //透传
                    {
                        ale = service.getCharacteristic(UUID
                                .fromString(Characteristic_uuid_TX));
                    }
                    break;
                    case 1:// 0xFFE2 //iBeacon_UUID
                    {
                        ale = service.getCharacteristic(UUID
                                .fromString(Characteristic_uuid_FUNCTION));
                    }
                    break;
                    default:
                        ale = service.getCharacteristic(UUID
                ...
                boolean set = mBluetoothGatt.setCharacteristicNotification(ale,
                        true);
                // Log.d(TAG," setnotification = " + set);
                BluetoothGattDescriptor dsc = ale.getDescriptor(UUID
            ...
            // {
            // BluetoothGattService service
            // =mBluetoothGatt.getService(UUID.fromString("49535343-fe7d-4ae5-8fa9-9fafd205e455"));
            // BluetoothGattCharacteristic ale
            // =service.getCharacteristic(UUID.fromString(Service_uuid));
            // boolean set = mBluetoothGatt.setCharacteristicNotification(ale,
            // false);
            // Log.d(TAG," setnotification = " + set);
            // BluetoothGattDescriptor dsc
            // =ale.getDescriptor(UUID.fromString(Characteristic_uuid_TX));
    ...
    public boolean set_password(String password, String new_password)// 读取设备密码
    {
        ...
        if (password == null || new_password == null)
            return false;
        // else if( password.length()!=6||new_password.length()!=6 )return
        // false;
    ...
    public boolean MC_Set_angle(String angle)//
    {
        ...
        if (angle_int_value <= 0x09) {
            ...
            WriteBytes = hex2byte(txt.toString().getBytes());
            /*
             * byte[] p1 = new byte[6]; p1[0] = (byte) 0xaa;// HEAD p1[1] =
             * (byte) ((0xaa+tx_cnt)^0x01);// CMD p1[2] = (byte)
             * ((0xaa+tx_cnt)^0x02);// LEN p1[3] = (byte)
             * ((0xaa+tx_cnt)^0xa1);// DATA p1[4] = (byte)
             * ((0xaa+tx_cnt)^0xf0);// CRC p1[5] =
             * (byte)(p1[0]^p1[1]^p1[2]^p1[3]^p1[4]); tx_cnt++; if( tx_cnt>=100
             * )tx_cnt=1;
             */
            BluetoothGattCharacteristic gg;
            gg = mBluetoothGatt.getService(UUID.fromString(Service_uuid))
    ...
    public boolean MC_set_button(boolean p)//
    {
        ...
        WriteBytes = hex2byte(txt.toString().getBytes());
        /*
         * byte[] p1 = new byte[6]; p1[0] = (byte) 0xaa;// HEAD p1[1] = (byte)
         * ((0xaa+tx_cnt)^0x01);// CMD p1[2] = (byte) ((0xaa+tx_cnt)^0x02);//
         * LEN p1[3] = (byte) ((0xaa+tx_cnt)^0xa1);// DATA p1[4] = (byte)
         * ((0xaa+tx_cnt)^0xf0);// CRC p1[5] =
         * (byte)(p1[0]^p1[1]^p1[2]^p1[3]^p1[4]); tx_cnt++; if( tx_cnt>=100
         * )tx_cnt=1;
         */
    ...
    public boolean set_ibeacon_UUID(String uuid)// 设置iBeacon UUID
    {
        if (uuid.length() == 36) {
            ...
            if (v1.equals("-") && v2.equals("-") && v3.equals("-")
                ...
                // "提示！UUID输入格式不对", Toast.LENGTH_SHORT);
                // toast.show();
                return false;
            }
        } else {
            // Toast toast = Toast.makeText(DeviceControlActivity.this,
            // "提示！UUID输入不对", Toast.LENGTH_SHORT);
            // toast.show();
            return false;
        }
    }
    ...
    public void set_PWM_ALL_pulse(int PWM1_pulse, int PWM2_pulse,
                                  int PWM3_pulse, int PWM4_pulse)// 设置PWM4脉宽
    {
        String txt = "E8a7";
    ...
    public void set_LED_Mode(int i)// 设备LED工作模式 设置LED灯模式 可设置--（面板模式 = 51、固定模式
    // =（1-25）、自定义模式 = 50
    {
        String txt = "E9b101";
        String vs = String.format("%02x", i);
    ...
    public int get_connected_status(List<BluetoothGattService> gattServices) {
        ...
        for (BluetoothGattService gattService : gattServices) {
            ...
            currentServiceData.put(LIST_NAME1,
            ...
            for (BluetoothGattCharacteristic gattCharacteristic : gattCharacteristics) {
                ...
                currentCharaData.put(LIST_NAME1,
        ...
        if (jdy_ble_ffe1 == 1 && jdy_ble_ffe2 == 1)
            return 2;// JDY-06,JDY-06
        else if (jdy_ble_ffe1 == 1 && jdy_ble_ffe2 == 0)
            return 1;// JDY-09,JDY-10
        else
            return 0;// 不为JDY系列BLE蓝牙
        // return count_char;
    }
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onCharacteristicRead(BluetoothGatt gatt,
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                if (UUID.fromString(Characteristic_uuid_TX).equals(
                        characteristic.getUuid())) {
                    broadcastUpdate(ACTION_DATA_AVAILABLE, characteristic);
                } else if (UUID.fromString(Characteristic_uuid_FUNCTION)
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt,
            ...
            if (UUID.fromString(Characteristic_uuid_TX).equals(
                    characteristic.getUuid())) {
                broadcastUpdate(ACTION_DATA_AVAILABLE, characteristic);
            } else if (UUID.fromString(Characteristic_uuid_FUNCTION).equals(
                    characteristic.getUuid())) {
                broadcastUpdate(ACTION_DATA_AVAILABLE1, characteristic);
            }
        }
    ...
    private void broadcastUpdate(final String action,
        ...
        } else if (UUID.fromString(Characteristic_uuid_TX).equals(
                characteristic.getUuid())) {
