 ...
 */
public class jdy_Activity extends Activity implements
			...
			switch (v.getId()) {
				case R.id.tx_button:// uuid1002 数传通道发送数据
					if (connect_status_bit) {
						...
						// Toast.LENGTH_LONG).show();
						Toast toast = Toast.makeText(jdy_Activity.this, "设备没有连接！",
								Toast.LENGTH_SHORT);
						toast.show();
					}
					break;
	...
	private void displayData1(byte[] data1) // 接收FFE2功能配置返回的数据
	{
		...
		{
			if (data1[1] == (byte) 0x01) {
				// Toast.makeText(jdy_Activity.this, "提示！APP密码连接成功",
				// Toast.LENGTH_SHORT).show();
			} else {
	...
	private void displayGattServices(List<BluetoothGattService> gattServices) {
				...
				&& mBluetoothLeService.get_connected_status(gattServices) == 1)// 表示为JDY-09、JDY-10系列蓝牙模块
		{
			...
			Toast toast = Toast.makeText(jdy_Activity.this,
