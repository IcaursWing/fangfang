...
public class jdy_switch_Activity extends Activity {
	...
	Button.OnClickListener listener = new Button.OnClickListener() {// 创建监听对象
		public void onClick(View v) {
			...
			case R.id.switch_password_write: {
				// mBluetoothLeService.set_password(
				// switch_pass_value.getText().toString(),
				// switch_new_pass_value.getText().toString() );
				switch_status_txt.setText("");
				if (mBluetoothLeService.set_name(switch_new_name_value
						.getText().toString()))// 支持中文蓝牙名
				{

				} else
					switch_status_txt.setText("设备名不能为空");
	...
	protected void onDestroy() {
		super.onDestroy();
		mBluetoothLeService.disconnect();
		// mBluetoothLeService.set_mem_data( mDeviceAddress,
		// switch_pass_value.getText().toString() );
	...
	private void displayData1(byte[] data1) // 接收FFE2功能配置返回的数据
	{
		...
		// Toast.makeText(jdy_switch_Activity.this, "rx:"+str,
		...
		if (data1.length == 5 && data1[0] == (byte) 0xf6)// 判断是否是读取IO状态位
		{
			...
			if (data1[1] == 0x01) {
				swt1_st = true;
				key1.setImageDrawable(getResources().getDrawable(
			...
			if (data1[2] == 0x01) {
				swt2_st = true;
				key2.setImageDrawable(getResources().getDrawable(
						R.drawable.switch_on));
			} else {
				swt2_st = false;
				key2.setImageDrawable(getResources().getDrawable(
			...
			if (data1[3] == 0x01) {
				swt3_st = true;
				key3.setImageDrawable(getResources().getDrawable(
						R.drawable.switch_on));
			} else {
				swt3_st = false;
				key3.setImageDrawable(getResources().getDrawable(
			...
			if (data1[4] == 0x01) {
				swt4_st = true;
				key4.setImageDrawable(getResources().getDrawable(
						R.drawable.switch_on));
			} else {
				swt4_st = false;
				key4.setImageDrawable(getResources().getDrawable(
		...
		if (data1.length == 2 && data1[0] == (byte) 0x61)// 更新设备名成功
		{
			if (data1[1] == (byte) 0x01) {
				...
				// Toast.makeText(jdy_ibeacon_Activity.this, "设备名更新成功",
				// Toast.LENGTH_SHORT).show();
			} else {
				switch_status_txt.setText("设备名更新失败");
				// Toast.makeText(jdy_ibeacon_Activity.this, "设备名更新未成功",
				// Toast.LENGTH_SHORT).show();
			}
	...
	private void displayGattServices(List<BluetoothGattService> gattServices) {
				...
				&& mBluetoothLeService.get_connected_status(gattServices) == 2)// 表示为JDY-06、JDY-08系列蓝牙模块
		{
			if (connect_status_bit) {
				...
				// Toast.LENGTH_LONG).show();
				Toast toast = Toast.makeText(jdy_switch_Activity.this,
