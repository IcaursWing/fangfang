...
public class ControlActivity extends Activity implements
	...
	protected void onDestroy() {
		...
		timer.cancel();
		timer = null;
		ControlActivity.this.unregisterReceiver(mGattUpdateReceiver);
	}
	...
	private void displayGattServices(List<BluetoothGattService> gattServices) {
				...
				&& mBluetoothLeService.get_connected_status(gattServices) == 2)// 表示为JDY-06、JDY-08系列蓝牙模块
		{
			...
			if (connect_status_bit) {
				...
				// Toast.makeText(this, "Deleted Successfully!",
				// Toast.LENGTH_LONG).show();
				Toast toast = Toast.makeText(ControlActivity.this, "设备没有连接！",
						Toast.LENGTH_SHORT);
				toast.show();
			}
	...
	class HandleReceivedMessage extends Thread {
		...
		public void run() {
			...
			case 0x15:
				...
				case 0x00:
					Toast.makeText(ControlActivity.this, "发送成功！",
							Toast.LENGTH_LONG).show();
					break;
				case 0x01:
					Toast.makeText(ControlActivity.this, "发送失败-参数空文件名。",
							Toast.LENGTH_LONG).show();
					break;
				case 0x02:
					Toast.makeText(ControlActivity.this, "发送失败-电池电量低。",
							Toast.LENGTH_LONG).show();
					break;
				case 0x03:
					Toast.makeText(ControlActivity.this, "动作不存在！",
							Toast.LENGTH_LONG).show();
					break;
