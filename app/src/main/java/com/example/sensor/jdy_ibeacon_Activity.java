...
public class jdy_ibeacon_Activity extends Activity {
	...
	TextView set_name_success_id, set_password_success_id,
			set_major_success_id, set_minor_success_id, set_uuid_success_id;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// color_arry=new
		// int[]{R.color.gray,R.color.bule,R.color.green,R.color.yellow};
		...
		ib_uuid_text = (EditText) findViewById(R.id.ib_uuid_text);// ibeacon
																	// uuid
		ib_mayjor_text = (EditText) findViewById(R.id.ib_mayjor_text);// ibeacon
																		// major
		ib_minor_text = (EditText) findViewById(R.id.ib_minor_text);// ibeacon
																	// minor
		...
		/*
		 * final Intent intent = getIntent(); mDeviceName =
		 * intent.getStringExtra(EXTRAS_DEVICE_NAME); mDeviceAddress =
		 * intent.getStringExtra(EXTRAS_DEVICE_ADDRESS);
		 * 
		 * // Sets up UI references. ((TextView)
		 ...
		 * 
		 * key1 = (ToggleButton)findViewById(R.id.toggleButton1); key2 =
		 * (ToggleButton)findViewById(R.id.toggleButton2); key3 =
		 * (ToggleButton)findViewById(R.id.toggleButton3); key4 =
		...
		public void onClick(View v) {
			...
			case R.id.password_write:// 更改密码
				if (connect_status_bit) {
					...
					// if( mBluetoothLeService.set_password(
					// pass_value.getText().toString(),
					// new_pass_value.getText().toString() )==false )
					// {
					// Toast.makeText(jdy_ibeacon_Activity.this,
					// "密码长度不对，密码必须为6位数字", Toast.LENGTH_SHORT).show();
					// }

					mBluetoothLeService.set_password(pass_value.getText()
							.toString(), new_pass_value.getText().toString());
					...
					// Toast.makeText(jdy_ibeacon_Activity.this, "ps:"+txt,
					...
					// Toast.LENGTH_LONG).show();
					Toast toast = Toast.makeText(jdy_ibeacon_Activity.this,
			...
			case R.id.ib_button:// 一键配置
			{
				...
				if (dev_password.equals(current_dev_password)) {
					...
					{
						set_uuid_success_id
								.setText("输入的iBeacon UUID与当前设备UUID相同无需更新");
					} else// 不相同更新
					{
						if (mBluetoothLeService.set_ibeacon_UUID(ib_uuid_text
								.getText().toString()) == false)
							set_uuid_success_id.setText("输入的iBeacon UUID格式有错！");
					}

					mBluetoothLeService.Delay_ms(20);
					if (current_major.equals(ib_mayjor_text.getText()
							.toString()))// 相同不更新
					{
						set_major_success_id
								.setText("输入的iBeacon major与当前设备major相同无需更新");
					} else// 不相同更新
					{
						if (mBluetoothLeService
								.set_ibeacon_MAJOR(ib_mayjor_text.getText()
										.toString()) == false)
					...
					mBluetoothLeService.Delay_ms(20);
					if (current_minor
							.equals(ib_minor_text.getText().toString()))// 相同不更新
					{
						set_minor_success_id
								.setText("输入的iBeacon minor与当前设备minor相同无需更新");
					} else// 不相同更新
					{
						if (mBluetoothLeService.set_ibeacon_MIMOR(ib_minor_text
								.getText().toString()) == false)
	...
	protected void onDestroy() {
		super.onDestroy();
		mBluetoothLeService.disconnect();
		mBluetoothLeService.set_mem_data(mDeviceAddress, pass_value.getText()
				.toString());
	...
	private void displayData1(byte[] data1) // 接收FFE2功能配置返回的数据
	{
		...
		// Toast.makeText(jdy_ibeacon_Activity.this, "function_rx:"+str1,
		// Toast.LENGTH_SHORT).show();
		...
		if (data1.length == 2 && data1[0] == (byte) 0x51)// 更新密码成功
		{
			if (data1[1] == (byte) 0x01) {
				...
				// Toast.makeText(jdy_ibeacon_Activity.this, "密码更新成功",
				// Toast.LENGTH_SHORT).show();
			} else {
		...
		} else if (data1.length == 7 && data1[0] == (byte) 0x52)// 读取密码
		{
			...
			// Toast.makeText(jdy_ibeacon_Activity.this,
			// "function_rx:"+current_dev_password, Toast.LENGTH_SHORT).show();
		} else if (data1.length == 2 && data1[0] == (byte) 0x61
				&& data1[1] == (byte) 0x01)// 更新蓝牙名成功
		{
			// Toast.makeText(jdy_ibeacon_Activity.this, "蓝牙名更新成功",
			// Toast.LENGTH_SHORT).show();
			set_name_success_id.setText("iBeacon 名更新成功");
		} else if (data1.length == 2 && data1[0] == (byte) 0x11
				&& data1[1] == (byte) 0x01)//
		{
			set_uuid_success_id.setText("iBeacon uuid更新成功");
		} else if (data1.length == 2 && data1[0] == (byte) 0x21
				&& data1[1] == (byte) 0x01)//
		{
			set_major_success_id.setText("iBeacon Major更新成功");
		} else if (data1.length == 2 && data1[0] == (byte) 0x31
				&& data1[1] == (byte) 0x01)//
		{
	...
	private void displayGattServices(List<BluetoothGattService> gattServices) {
				...
				&& mBluetoothLeService.get_connected_status(gattServices) == 2)// 表示为JDY-06、JDY-08系列蓝牙模块
		{
			if (connect_status_bit) {
				...
				mBluetoothLeService.get_password(pass_value.getText()
						.toString());
